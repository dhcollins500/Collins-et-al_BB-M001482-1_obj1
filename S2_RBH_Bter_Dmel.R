...
# File last updated: 26.05.20
# Project: Gene expression in larval caste determination of bumblebees and the transition to advanced eusociality
# Analysis: Gene ontology enrichment analysis
# Subsection: Generating a list of likely B. terrestris and D. melanogaster 
# orthologs using reciprocal best hit (RBH) BLAST.
# Tasks: Import BLAST results, calculates RBH BLAST orthologs
# IMPORTANT NOTE: Value of "a" needs to be changed to be specific for your
# computer.
#-------------------------------------------------------------------------------
# Inputs:
# Output (tabular with comment lines format) from BLASTp of B. terrestris
# proteins BLASTed against D. melanogaster proteins, and vice versa.

# Outputs:
# A text file with the results of the RBH analysis of B. terrestris and 
# D. melanogaster proteins, and a .csv where these results have been annotated
# with corresponding gene information.
#-------------------------------------------------------------------------------
...
# Calculating RBH BLAST Orthologs Between B. terrestris and D. melanogaster 
# ----

# Setting input values for the function.

x <- "42_BlastOutput_Dmel_protein_on_Bt_protein"  

y <- "41_BlastOutput_Bt_protein_on_Dmel_protein"

z <- "10_Bter_to_Dmel_RBH_Results"
...
# Reading the Results of the Function into an Object ----

# Setting working directory and list available files.

setwd(paste(a, "/04_analysis_output", sep = ""))

dir()

# Reading in results generated above.

resultsName <- 
  paste("Bter_to_Dmel_RBH_Results_", Sys.Date(), ".txt", sep = "")

RBHResults <- read.table(resultsName)

# Renaming the columns

colnames(RBHResults) <- c("Bter_protein_ID", "Dmel_protein_ID")

# Adding Gene IDs and Names to the Results ----

# Isolating D. melanogaster genome information from annotation package.

dmelKeys <- keys(org.Dm.eg.db, keytype = "ENTREZID")  # place keys in an object

dmelAnnotations <-
  select(org.Dm.eg.db, columns = c("FLYBASE", "FLYBASEPROT", "GENENAME"),
         keys = dmelKeys)

dmelAnnotations <- dmelAnnotations[, c("FLYBASE", "FLYBASEPROT", "GENENAME")]
# removing the ENTREZID column.

# Renaming the columns.

colnames(dmelAnnotations) <- 
  c("Dmel_gene_ID", "Dmel_protein_ID", "Dmel_gene_description")

# Merging the Dmel annotations with the RBH results.

RBHResultsDmelNames <- 
  merge(RBHResults, dmelAnnotations, by = "Dmel_protein_ID", all = TRUE)
...
# Merging the Bter annotations with the RBH results/Dmel annotations.

RBHDmelBter <- merge(RBHResultsDmelNames, bombusProteinAnnotations,
                     by = "Bter_protein_ID", all = TRUE)

# Removing cases where Bter_protien_ID or Dmel_protein_ID = NA.

RBHDmelBter <- 
  subset(RBHDmelBter, !is.na(Bter_protein_ID) & !is.na(Dmel_protein_ID) & !is.na(Dmel_gene_ID) & !is.na(Bter_gene_ID) )

# Outputting the Results to a File ----

outputName <- "Bter_to_Dmel_RBH_Annotated.csv"

write.csv(RBHDmelBter, outputName, row.names = FALSE)
